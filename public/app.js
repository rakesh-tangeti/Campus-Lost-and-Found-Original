...
function openReport(type){
 ...
 <div class="field"><label>Item name</label><input id="item" name="item" required placeholder="e.g. Black wallet, keys, ID card"></div>
 <div class="field"><label>Category</label><select id="category" name="category" required><option value="">Select category</option><option>Electronics</option><option>Documents / ID</option><option>Keys</option><option>Wallet / Money</option><option>Bag / Accessories</option><option>Other</option></select></div>
 <div class="field"><label>Location</label><input id="location" name="location" required placeholder="Library, Canteen, Block A"></div>
 <div class="field"><label>Date & time</label><input id="date" name="date" type="datetime-local" required></div>
 <div class="field"><label>Description</label><textarea id="description" name="description" required placeholder="Color, brand, unique marks"></textarea></div>
 <div class="field"><label>Your name</label><input id="name" name="name" required></div><div class="field"><label>Contact / email</label><input id="contact" name="contact" required></div>
...
async function submitReport(e,type){
 ...
 try {
  const form=e.target;
  const value=(name)=>{ const el=form.elements.namedItem(name); return el ? String(el.value || "").trim() : ""; };
  const data={type,item:value("item"),category:value("category"),location:value("location"),date:value("date"),description:value("description"),name:value("name"),contact:value("contact")};
  const res=await fetch("/api/reports",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)});
  const out=await res.json().catch(()=>({error:"Server returned an invalid response"}));
 ...
 } catch(err) { console.error("Report submission error:",err); alert(err.message||"Could not submit report. Please try again."); }
...
async function login(e){
 e.preventDefault();
 try {
  const r=await fetch("/api/security/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:String(document.getElementById("email")?.value||"").trim(),password:String(document.getElementById("password")?.value||"")})});
  const x=await r.json().catch(()=>({error:"Server returned an invalid response"}));
  if(!r.ok) throw new Error(x.error||`Login failed (${r.status})`);
  securityToken=x.token; localStorage.setItem("securityToken",securityToken);
  closeModal(); document.getElementById("home").classList.add("hidden"); document.getElementById("security").classList.remove("hidden");
  socket.emit("joinSecurity",securityToken); loadSecurity();
 } catch(err){ console.error("Security login error:",err); alert(err.message||"Login failed. Please try again."); }
}
...
socket.on("stats",s=>{if(securityToken){document.getElementById("sLost").textContent=s.lost;document.getElementById("sFound").textContent=s.found;document.getElementById("sMatch").textContent=s.matches;document.getElementById("sPending").textContent=s.pending}});
